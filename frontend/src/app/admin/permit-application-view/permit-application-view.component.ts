...
export class PermitApplicationViewComponent implements OnInit {
  ...
  updateApplicationStatus(application, status) {
    ...
    this.applicationService.update(application, this.type).subscribe(
      (data: any) => {
        if (status === 'Accepted') {
          this.alertService.addSuccessMessage('Application has been sent to SUDS for further processing and an email with your message has been sent to the applicant.');
        }
        if (status === 'Hold') {
          this.alertService.addSuccessMessage('Permit application successfully put on hold and an email with your message has been sent to the applicant.');
        }
        if (status === 'Returned') {
          this.alertService.addSuccessMessage('Permit application successfully rejected and an email with your message has been sent to the applicant.');
        }
