...
describe('christmas tree admin controller', () => {
  ...
  describe('.getPermitSummaryReport', () => {
    ...
    describe('for a single forest', () => {
      ...
      it('returns report containing only permits for authorized forest, start and end date', (done) => {
        const forestId = authorizedForest.id;
        getPermitSummaryReport(forestId, today, tomorrow)
          .expect('Content-Type', /json/)
          .expect(({ body }) => {
            expect(body.numberOfPermits).to.equal(1);
            const permitNumbers = body.permits.map(permit => permit.permitNumber);
            expect(permitNumbers).to.include(String(authorizedPermit.permitNumber));
    ...
    describe('for ALL forests', () => {
      ...
      it('includes only permits that which the user is authorized to see', (done) => {
        getPermitSummaryReport(permitId, today, tomorrow)
          .expect('Content-Type', /json/)
          .expect(({ body }) => {
            expect(body.numberOfPermits).to.equal(2);
            const permitNumbers = body.permits.map(permit => permit.permitNumber);
            expect(permitNumbers).to.include(String(authorizedPermit.permitNumber));
            expect(permitNumbers).to.include(String(authorizedPermit2.permitNumber));
            expect(permitNumbers).not.to.include(String(unauthorizedPermit.permitNumber));
          })
          .expect(200, done);
      });
